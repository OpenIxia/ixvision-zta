...
"$PYENV_DIR/ixvision-ztp/ixvision_ztp.py"  -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP
...
"$PYENV_DIR/ixvision-ztp/ixvision_lldp_tag.py"  -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -t TAP,SPAN,monitor,capture
...
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_port_group.py"  -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -t tap -n TAPs -m net
echo "Create/update network port group SPANs with ports tagged as SPAN"
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_port_group.py"  -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -t span -n SPANs -m net

# Tool side - monitor ports as BRO LBG
echo "Create/update load-balance tool port group BRO with ports tagged as monitor"
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_port_group.py"  -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -t monitor -n BRO -m lb

echo "Create/update load-balance tool port group MOLOCH with ports tagged as capture"
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_port_group.py"  -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -t capture -n MOLOCH -m lb
...
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_filter.py" -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -n "IoC_Detection" -i TAPs -o BRO -m all
echo "Connect IoC_Detection filter to SPANs as well"
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_filter.py" -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -n "IoC_Detection" -i SPANs -o BRO  -m all

echo "Create IPv4_Capture filter for traffic from TAPs"
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_filter.py" -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -n "IPv4_Capture" -i TAPs -o MOLOCH -m pbc -c moloch-ipv4.json
echo "Connect IPv4_Capture filter to SPANs as well"
"$PYENV_DIR/ixvision-ztp/ixvision_ztp_filter.py" -u $WEB_API_USERNAME -p $WEB_API_PASSWORD -h $DEVICE_IP -n "IPv4_Capture" -i SPANs -o MOLOCH -m pbc -c moloch-ipv4.json
